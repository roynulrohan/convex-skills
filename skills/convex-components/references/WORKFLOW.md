...
export const onboardingWorkflow = workflow.define({
  ...
  returns: v.null(), // Optional: runtime validation of return value
...
For type-safe event payloads, use `defineEvent`:
...
// Send the typed event from another function:
await workflow.sendEvent(ctx, {
  ...approvalEvent,
  workflowId,
  value: { approved: true, choice: 0 },
});
```

#### Send Error Events

Fail a waiting workflow step by sending an error:
...
export const getWorkflowStatus = action({
  ...
  handler: async (ctx, { workflowId }) => {
    ...
    // status.type: "inProgress" | "completed" | "failed" | "canceled"
...
```typescript
export const getWorkflowSteps = action({
  args: { workflowId: vWorkflowId },
  handler: async (ctx, { workflowId }) => {
    return await workflow.listSteps(ctx, workflowId, {
      order: "asc",
      paginationOpts: { numItems: 50, cursor: null },
    });
    // Returns: { page: Step[], continueCursor, isDone }
    // Each step has: name, args, runResult, inProgress, completedAt, createdAt
  },
});
```
...
Remove completed workflow data from the database:
