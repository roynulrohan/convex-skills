...
export const rateLimiter = new RateLimiter(components.rateLimiter, {
  ...
  llmTokens: {
    ...
    rate: 40000,
    period: MINUTE,
    shards: 10, // Reduces OCC conflicts
  },
});
```
...
```typescript
// For scheduled work - reserve capacity now, run when available
...
if (status.retryAfter) {
  ...
  await ctx.scheduler.runAfter(status.retryAfter, internal.process.doWork, {
    ...args,
    skipCheck: true,
  });
  return;
...
In the retried function, skip the rate limit check:

```typescript
export const doWork = internalMutation({
  args: { /* ... */ skipCheck: v.optional(v.boolean()) },
  handler: async (ctx, args) => {
    if (!args.skipCheck) {
      await rateLimiter.limit(ctx, "llmRequests", {
        key: args.userId,
        throws: true,
      });
    }
...
Define a rate limit inline without pre-registering it in the constructor:

```typescript
const config = { kind: "fixed window" as const, rate: 1, period: SECOND };
const status = await rateLimiter.limit(ctx, "oneOffName", { config });
```

This is useful when the rate limit parameters are determined at runtime or when you don't want to pre-define every limit.
