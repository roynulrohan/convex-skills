...
> For the `@convex-dev/migrations` component (stateful online migrations with tracking, resumability, and CLI runner), see `convex-components/references/MIGRATIONS.md`.
...
The general pattern for any schema change is:

1. Make the schema change backward-compatible (use optional fields)
2. Update code to handle both old and new data
3. Backfill existing documents in batches
4. Once backfill completes, tighten the schema (make fields required, remove old fields)
...
An alternative backfill approach using `.filter()` instead of pagination (simpler for smaller datasets):
...
For applications with multiple migrations, create a reusable migration system with a tracking table.
...
### Example Migration Using the Runner
    ...
    try {
      ...
      if (migration) {
        await ctx.db.patch(migrationId, {
          processed: (migration.processed ?? 0) + processed,
        });
      }
